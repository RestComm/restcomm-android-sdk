...
public class SignalingClient extends Handler {
   ...
   static private SignalingHandlerThread signalingHandlerThread;
   static private Handler signalingHandler;
   //UIHandler uiHandler;
...
*/
    ...
    */
   static public String open(SignalingClientListener listener, Context context, HashMap<String, Object> parameters)
   {
      RCLogger.e(TAG, "--- open(), initialized: " + initialized + ", isClosing: " + isClosing);
      if (initialized && !isClosing) {
         throw new RuntimeException("SignalingClient already initialized");
      }

      SignalingClient.context = context;
      listener = listener;
      parameters = parameters;

      if (isClosing) {
         RCLogger.e(TAG, "--- open(), setting pendingInitialize");
         pendingInitialize = true;
         return "";
      }

      // create signaling handler thread and handler/signal
      signalingHandlerThread = new SignalingHandlerThread(SignalingClient.this);
      signalingHandler = signalingHandlerThread.getHandler();
    ...
    * @return The jobId for the new job created at Signaling thread
    */
   static public String reconfigure(HashMap<String, Object> parameters)
   {
      String jobId = generateId();
    ...
    */
   static public void call(String jobId, HashMap<String, Object> parameters)
   {
    ...
    */
   static public void accept(String jobId, HashMap<String, Object> parameters)
   {
    ...
    */
   static public void disconnect(String jobId, String reason)
   {
    ...
    */
   static public void sendDigits(String jobId, String digits)
   {
    ...
    * @return
    */
   static public String sendMessage(HashMap<String, Object> parameters)
   {
      String jobId = generateId();
   ...
   public void handleMessage(Message inputMessage)
   {
      ...
      if (message.type == SignalingMessage.MessageType.OPEN_REPLY && listener != null) {
         ...
         RCLogger.e(TAG, "--- handleMessage(): CLOSE_REPLY, pendingInitialize: " + pendingInitialize);
         signalingHandlerThread.quit();
         initialized = false;
         isClosing = false;
         //closePending = false;
         if (listener != null) {
            if (!pendingInitialize) {
               ...
               open(listener, context, parameters);

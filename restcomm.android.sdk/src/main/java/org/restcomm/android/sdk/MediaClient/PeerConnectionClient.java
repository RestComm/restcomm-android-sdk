...
public class PeerConnectionClient {
   ...
   private void createPeerConnectionFactoryInternal(Context context)
   {
      ...
      RCLogger.d(TAG, "Create peer connection factory. Use video: " +
      ...
      RCLogger.d(TAG, "Pereferred video codec: " + preferredVideoCodec);
      ...
      if (!peerConnectionParameters.useOpenSLES) {
         RCLogger.d(TAG, "Disable OpenSL ES audio even if device supports it");
         WebRtcAudioManager.setBlacklistDeviceForOpenSLESUsage(true /* enable */);
      }
      else {
         RCLogger.d(TAG, "Allow OpenSL ES audio if device supports it");
   ...
   private void createPeerConnectionInternal(EglBase.Context renderEGLContext)
   {
      ...
      if (videoCallEnabled) {
         RCLogger.d(TAG, "EGLContext: " + renderEGLContext);
    ...
    Logging.enableTracing(
      ...
      if (peerConnectionParameters.aecDump) {
         try {
            ...
            RCLogger.e(TAG, "Can not open aecdump file", e);
         }
      }
   ...
   */
   ...
   private void changeCaptureFormatInternal(int width, int height, int framerate)
   {
      if (!videoCallEnabled || isError || videoCapturer == null) {
         RCLogger.e(TAG, "Failed to change capture format. Video: " + videoCallEnabled + ". Error : "
               + isError);
         return;
      }
      RCLogger.d(TAG, "changeCaptureFormat: " + width + "x" + height + "@" + framerate);
   ...
   private class PCObserver implements PeerConnection.Observer {
      ...
      {
         RCLogger.d(TAG, "IceConnectionReceiving changed to " + receiving);
      }
